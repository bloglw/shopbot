...
def update_available_balance(tg_id, amount, transaction_hash, usdt_address=None, timestamp=None):
    try:
        conn = sqlite3.connect('data.sqlite3')
        cursor = conn.cursor()

        # 检查充值记录是否已经处理过
        cursor.execute("SELECT COUNT(*) FROM recharge_logs WHERE transaction_hash=? AND processed=1", (transaction_hash,))
        if cursor.fetchone()[0] > 0:
            print(f"Transaction {transaction_hash} already processed.")
            return

        # 更新用户余额
        cursor.execute("SELECT available_balance FROM user WHERE tg_id=?", (tg_id,))
        current_balance = cursor.fetchone()[0]
        new_balance = current_balance + amount
        cursor.execute("UPDATE user SET available_balance=? WHERE tg_id=?", (new_balance, tg_id))

        # 插入充值记录并标记为已处理
        cursor.execute("INSERT INTO recharge_logs (tg_id, usdt_address, transaction_hash, amount, timestamp, processed) VALUES (?, ?, ?, ?, ?, 1)",
                       (tg_id, usdt_address, transaction_hash, amount, timestamp or int(time.time())))

        conn.commit()
        print(f"User {tg_id} balance updated successfully.")
    except Exception as e:
